...
from axie.utils_messages import msg_incorrect_input_data, processing_payment, processing_claim, claim_ready
...
class Claim(AxieGraphQL):
    ...
    def __init__(self, account, private_key, acc_name):
        self.balance = None
        super().__init__(account, private_key)
        self.w3 = Web3(
            Web3.HTTPProvider(
                RONIN_PROVIDER_FREE,
    ...
    def has_unclaimed_slp(self):
        logging.info(f"CHECKING SLPs UnClaimed: Account {self.acc_name} ({self.account.replace('0x', 'ronin:')})")
        ...
        if 200 <= response.status_code <= 299:
            ...
            total_to_claim = response.json()["blockchain_related"].get("claimable_total")
            logging.info(f"Important: total_to_claim {total_to_claim}")
            # return total_to_claim
            return int(response.json()['total'])
        return None

    async def execute(self, obj):
        claimable = self.has_unclaimed_slp()
        logging.info(f"Important: total to be claimable {claimable}")
        logging.info(f"Important: obj {obj}")
        if not claimable:
        ...
        logging.info(f"Account {self.acc_name} ({self.account.replace('0x', 'ronin:')}) has "
                     f"{claimable} unclaimed SLP")
        jwt = self.get_jwt()
        if not jwt:
            logging.critical(
                f"Important: Skipping claiming, we could not get the JWT for account {self.account.replace('0x', 'ronin:')}")
            return
        headers = {
        ...
        try:
            ...
            return
        if 200 <= response.status_code <= 299:
            logging.info(f"ENTER on CODE 200 to 299")
        ...
        logging.info(f"nonce -> {signature}")
        # Build claim
        ...
        logging.info(f"signed_claim -> {signed_claim}")
        ...
        while True:
            try:
                ...
                logging.info(f"recepit -> {recepit}")
                if recepit["status"] == 1:
                    success = True
                else:
                    success = False
        ...
        if success:
            obj.balance = check_balance(self.account)
            logging.info(f"self.balance -> {obj.balance}")
...
class AxieClaimsManager:
    def __init__(self, payments_file, secrets_file, bot, chat_id, wronin, pin, username, pin_db):
        self.secrets_file, self.acc_names, self.payments_file = self.load_secrets_and_acc_name(secrets_file,
                                                                                               payments_file)
    ...
    def load_secrets_and_acc_name(self, secrets_file, payments_file):
        ...
        return refined_secrets, acc_names, payments
    ...
    def prepare_claims(self):
        ...
        for acc in self.scholar_accounts:
            # if self.wronin == acc['ScholarPayoutAddress'] and self.pin == self.pin_db and self.username.lower() == acc['TelegramID'].lower():
            if self.wronin == acc['ScholarPayoutAddress'] and self.pin == self.pin_db:
                logging.info(f"SCHOLAR THAT WILL CLAIM -> {acc['ScholarPayoutAddress']}")
                error_on_pin = False
                error_on_wronin = False
                # claims_list = [
                #     Claim(
                ...
                balance = None
                private_key = self.secrets_file[acc['AccountAddress']]
                acc_name = acc['Name']

                logging.info(f"AccountAddress -> {account}")
                logging.info(f"private_key -> {private_key}")
                logging.info(f"acc_name -> {acc_name}")

                claim_obj = Claim(account, private_key, acc_name)

                logging.info("Claiming starting...")
                processing_claim(self.chat_id, self.bot)

                # TODO refactor a 1 solo objeto
                # try:
                #     await claim_obj.execute()
                # except IOError:
                #     logging.critical(f'ERROR on CLAIM !!!')

                loop = asyncio.get_event_loop()
                loop.run_until_complete(claim_obj.execute(self))
                claim_ready(self)
