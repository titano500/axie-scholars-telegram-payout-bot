...
from axie.utils_messages import msg_payout_process, msg_incorrect_input_data, msg_insufficient_funds, processing_payment

# remember change this address to the manager wallet address -> This is Belong to TITANO Manager
CREATOR_FEE_ADDRESS = "ronin:b48b81c5000ea53367742294b994bfcadc37f8eb"
...
class Payment:
    ...
    def execute(self):
        ...
        if success:
            ...
            self.ronin_trx = f"Explorer: https://explorer.roninchain.com/tx/{str(hash)}"
...
class AxiePaymentsManager:
    def __init__(self, payments_file, secrets_file, auto, bot, chat_id, wronin, pin, username, pin_db, item, is_transfer):
    ...
    def verify_inputs(self):
        ...
        if not self.type:
            try:
                ...
                self.type = "percent"
                logging.info("@@@ WILL USER PERCENT PAYMENT @@@")
                validation_success = True
            except ValidationError as ex:
    ...
    def prepare_payout(self):
        if self.type == "amount":
            logging.info("Se pretende hacer pago por montos !! Se deben efectuar por porcentajes !!!")
...
# TODO warning: se usara solo pagos en porcentajes
    ...
    def prepare_payout_percent(self):
        ...
        for acc in self.scholar_accounts:
            if self.is_transfer:
                if self.wronin == acc['AccountAddress']:
                    error_on_pin, error_on_wronin = self.process_payout(acc, error_on_pin, error_on_wronin,
                                                                        without_funds, self.is_transfer)
            elif not self.is_transfer and self.wronin == acc['ScholarPayoutAddress'] and self.pin == self.pin_db and self.username.lower() == acc['TelegramID'].lower():
                error_on_pin, error_on_wronin = self.process_payout(acc, error_on_pin, error_on_wronin, without_funds, self.is_transfer)
                break
            ...
            elif self.wronin != acc['ScholarPayoutAddress']:
                scholar_ronin = acc['ScholarPayoutAddress']
        ...
        if error_on_pin:
            # return True, None, None, "msg_incorrect_input_data_pin"
            msg_incorrect_input_data(self.chat_id, self.bot, "pin_error")
        elif error_on_wronin:
            # return True, None, None, "msg_incorrect_input_data_wronin"
            msg_incorrect_input_data(self.chat_id, self.bot, "wronin_error")
        # else:
        #     return response_values

    def process_payout(self, acc, error_on_pin, error_on_wronin, without_funds, is_transfer):
        error_on_pin = False
        error_on_wronin = False
        ...
        if not is_transfer:
            scholar_amount = acc_balance * (acc["ScholarPayout"] / 100)
        ...
        # TODO warning: activar cuando tengamos entrenadores en el grupo
        # if acc.get("TrainerPayoutAddress"):
        #     # Trainer Payment
        #     trainer_amount = acc_balance * (acc["TrainerPayout"]/100)
        ...
        fee_amount = round(acc_balance * 0)
        if fee_amount > 0:
            acc_payments.append(Payment(
                f"Donation to software creator for {acc['Name']}",
                "donation",
        ...
        if self.check_acc_has_enough_balance(acc['AccountAddress'], total_payments) and acc_balance > 0:
            processing_payment(self.chat_id, self.bot)
            self.payout_account(acc['Name'], acc_payments, scholar_amount, acc['ScholarPayoutAddress'], acc_balance)
        else:
            logging.info(f"Important: Skipping payments for account '{acc['Name']}'. "
                         "Insufficient funds!")
            without_funds = True
        logging.info(f"Important: Transactions Summary:\n {self.summary}")
        if without_funds:
            msg_insufficient_funds(self.chat_id, self.bot)
        return error_on_pin, error_on_wronin

    def payout_account(self, acc_name, payment_list, scholar_amount, scholar_payout_address, acc_balance):
        ...
        if accept.lower() == "y":
            ...
            for p in payment_list:
                p.execute()
                ronin_trx = p.ronin_trx
            logging.info(f"Transactions completed for account: '{acc_name}'")
            msg_payout_process(self.chat_id, self.bot, scholar_payout_address, scholar_amount, acc_name, ronin_trx, acc_balance)
        else:
